 ...
 * 当有循环依赖的场景，当bean实例化好之后，为了防止有循环依赖，会提前暴露回调方法，
...
public class CustomSmartInstantiationAwareBeanPostProcessor implements SmartInstantiationAwareBeanPostProcessor {
    Logger logger = LoggerFactory.getLogger(CustomSmartInstantiationAwareBeanPostProcessor.class);
    @Override
    public Class<?> predictBeanType(Class<?> beanClass, String beanName) throws BeansException {
        if("school".equalsIgnoreCase(beanName)){
            logger.info("发生在postProcessBeforeInstantiation之前,用于预测beanClass，{}",beanClass);
        }
        return beanClass;
    }
    ...
    public Constructor<?>[] determineCandidateConstructors(Class<?> beanClass, String beanName) throws BeansException {
        if("school".equalsIgnoreCase(beanName)){
            logger.info("在postProcessBeforeInstantiation之后，用于确定该bean的构造函数之用，返回的是该bean的所有构造函数列表，{}",beanClass);
        }
    ...
    public Object getEarlyBeanReference(Object bean, String beanName) throws BeansException {
        if("school".equalsIgnoreCase(beanName)){
            logger.info("发生在postProcessAfterInstantiation之后，当有循环依赖的场景，当bean实例化好之后，为了防止有循环依赖，会提前暴露回调方法，，{}",bean);
        }
        return bean;
    }
}
